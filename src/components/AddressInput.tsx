...
export default function AddressInput({ label, name, required = true, defaultValue = "", onChange }: Props) {
  ...
  return (
    <div ref={boxRef} className="relative">
      ...
      {coords.lat != null && (
        <p className="mt-1 text-xs text-ok">✓ Address pinned — exact distance pricing</p>
      )}
      ...
      {open && suggestions.length > 0 && (
        <ul className="absolute z-20 mt-1 max-h-56 w-full overflow-auto rounded-lg border border-line bg-surface shadow-lg">
          {suggestions.map((s, i) => (
            <li key={i}>
              <button
                type="button"
